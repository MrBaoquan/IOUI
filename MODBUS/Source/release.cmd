...
copy %IOUI_SRC_PATH%IOUI-%PLATFORM%-MODBUS.dll %IOUI_DST_PATH% /y
copy %IOUI_SRC_CORE_PATH%modbus.dll %IOUI_DST_CORE_PATH% /y

if %CONFIGURE% equ Debug (
	xcopy %ProjectDir%Config\MODBUS %IOUI_SRC_PATH%ExternalLibraries\Config\MODBUS\ /E /F /Y
) else (
	xcopy %ProjectDir%Config\MODBUS %IOUI_DST_PATH%Config\MODBUS\ /E /F /Y
)
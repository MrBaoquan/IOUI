...
{
	static auto _serialPort = g_serialPorts[deviceIndex];
	static int _sResolution = g_sResolutions[deviceIndex];
	...
	while (_recvDatas.size() >= 10) {
		...
		int _raw = _recvDatas[3] << 8 | _recvDatas[4] & 1;
		int _angle = _raw * 360 / _sResolution;

		static short _lastAngle = SHORT_MAX;
		...
		//OutputDebugStringA(std::to_string(_deltaAngel).data());
		//OutputDebugStringA("\r\n");

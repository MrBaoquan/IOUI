...
copy %IOUI_SRC_PATH%IOUI-%PLATFORM%-NET-IOFE.dll %IOUI_DST_PATH% /y

if %CONFIGURE% equ Debug (
	xcopy %ProjectDir%Config\NET-IOFE %IOUI_SRC_PATH%ExternalLibraries\Config\NET-IOFE\ /E /F /Y
) else (
	xcopy %ProjectDir%Config\NET-IOFE %IOUI_DST_PATH%Config\NET-IOFE\ /E /F /Y
)
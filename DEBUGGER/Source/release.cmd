...
copy %IOUI_SRC_PATH%IOUI-%PLATFORM%-DEBUGGER.dll %IOUI_DST_PATH% /y

if %CONFIGURE% equ Debug (
	xcopy %ProjectDir%Config\DEBUGGER %IOUI_SRC_PATH%ExternalLibraries\Config\DEBUGGER\ /E /F /Y
) else (
	xcopy %ProjectDir%Config\DEBUGGER %IOUI_DST_PATH%Config\DEBUGGER\ /E /F /Y
)
...
copy %IOUI_SRC_PATH%IOUI-%PLATFORM%-NET-BRIDGE.dll %IOUI_DST_PATH% /y

if %CONFIGURE% equ Debug (
	xcopy %ProjectDir%Config\NET-BRIDGE %IOUI_SRC_PATH%ExternalLibraries\Config\NET-BRIDGE\ /E /F /Y
) else (
	xcopy %ProjectDir%Config\NET-BRIDGE %IOUI_DST_PATH%Config\NET-BRIDGE\ /E /F /Y
)

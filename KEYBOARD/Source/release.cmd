...
copy %IOUI_SRC_PATH%IOUI-%PLATFORM%-KEYBOARD.dll %IOUI_DST_PATH% /y

if %CONFIGURE% equ Debug (
	xcopy %ProjectDir%Config\KEYBOARD %IOUI_SRC_PATH%ExternalLibraries\Config\KEYBOARD\ /E /F /Y
) else (
	xcopy %ProjectDir%Config\KEYBOARD %IOUI_DST_PATH%Config\KEYBOARD\ /E /F /Y
)

...
copy %IOUI_SRC_PATH%IOUI-%PLATFORM%-SERIAL-BRIDGE.dll %IOUI_DST_PATH% /y

if %CONFIGURE% equ Debug (
	xcopy %ProjectDir%Config\SERIAL-BRIDGE %IOUI_SRC_PATH%ExternalLibraries\Config\SERIAL-BRIDGE\ /E /F /Y
) else (
	xcopy %ProjectDir%Config\SERIAL-BRIDGE %IOUI_DST_PATH%Config\SERIAL-BRIDGE\ /E /F /Y
)

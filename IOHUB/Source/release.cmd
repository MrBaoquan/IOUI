...
copy %IOUI_SRC_PATH%IOUI-%PLATFORM%-IOHUB.dll %IOUI_DST_PATH% /y

if %CONFIGURE% equ Debug (
	xcopy %ProjectDir%Config\IOHUB %IOUI_SRC_PATH%ExternalLibraries\Config\IOHUB\ /E /F /Y
) else (
	xcopy %ProjectDir%Config\IOHUB %IOUI_DST_PATH%Config\IOHUB\ /E /F /Y
)
...
copy %IOUI_SRC_PATH%IOUI-%PLATFORM%-SNAP7.dll %IOUI_DST_PATH% /y
copy %IOUI_SRC_CORE_PATH%snap7.dll %IOUI_DST_CORE_PATH% /y

if %CONFIGURE% equ Debug (
	xcopy %ProjectDir%Config\SNAP7 %IOUI_SRC_PATH%ExternalLibraries\Config\SNAP7\ /E /F /Y
) else (
	xcopy %ProjectDir%Config\SNAP7 %IOUI_DST_PATH%Config\SNAP7\ /E /F /Y
)